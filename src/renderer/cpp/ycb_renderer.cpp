...
class CppRenderer{
public:
    CppRenderer(int w, int h, int d):m_windowHeight(h),m_windowWidth(w),m_renderDevice(d) {};
...
PYBIND11_MODULE(CppRenderer, m) {
    py::class_<CppRenderer>(m, "CppRenderer")
        .def(py::init<int, int, int>())
        .def("init", &CppRenderer::init)
        .def("query", &CppRenderer::query)
        .def("map_tensor", &CppRenderer::map_tensor)
        .def("draw", &CppRenderer::draw, py::arg().noconvert())
        .def("release", &CppRenderer::release)
        .def("draw_py", &CppRenderer::draw_py, py::arg().noconvert());
